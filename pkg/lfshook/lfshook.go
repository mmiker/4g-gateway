// Package lfshook is hook for sirupsen/logger that used for writing the logs to local files.
package lfshook
...
var defaultFormatter = &logger.TextFormatter{DisableColors: true}
...
type PathMap map[logger.Level]string
...
type WriterMap map[logger.Level]io.Writer
...
func NewHook(output interface{}, formatter logger.Formatter) *LfsHook {
	hook := &LfsHook{
		lock: new(sync.Mutex),
	}
...
func (hook *LfsHook) SetFormatter(formatter logger.Formatter) {
	hook.lock.Lock()
	...
	if formatter == nil {
		...
		case *logger.TextFormatter:
			textFormatter := formatter.(*logger.TextFormatter)
...
func (hook *LfsHook) Fire(entry *logger.Entry) error {
	hook.lock.Lock()
	defer hook.lock.Unlock()
...
func (hook *LfsHook) ioWrite(entry *logger.Entry) error {
	var (
		writer io.Writer
		msg    []byte
		err    error
...
func (hook *LfsHook) fileWrite(entry *logger.Entry) error {
	var (
		fd   *os.File
		path string
		msg  []byte
...
func (hook *LfsHook) Levels() []logger.Level {
